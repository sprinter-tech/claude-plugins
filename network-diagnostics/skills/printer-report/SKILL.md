...
ambiguous). Never use a network name where a `network_id` is required. When the
user belongs to only one org, org labels are unnecessary.

If the user did not mention a network, you can get the `network_id` from
the `show_device` result in the next step (its result also carries the org).
